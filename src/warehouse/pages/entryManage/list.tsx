...
const EntryManageList: React.FC<PropsWithChildren<{ $apis }>> = ({ $apis }) => {
  ...
  const getEntryLogsList = async (formValue: {[propsName: string]: any}) => {
    try {
        ...
        {detail => (
          ...
          <View className="card_index_message">
            <View className="message_info">
              ...
              <View className="desc">入库类型：{detail.type}</View>
            </View>
          </View>
          <View className="card_operate">
            <UsButton
              size="mini"
              onClick={() => toEntryLogUpdate('detail', detail.id)}
            >查看</UsButton>
            <UsButton
              size="mini"
              theme="danger"
